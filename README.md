...
# 编辑 .env，填入真实 DASHSCOPE_API_KEY；不用 LangSmith 时保持追踪为 false
...
> ⚠️ 当前鉴权与多租户隔离尚未完成。Compose 默认只监听 `127.0.0.1`；在认证阶段完成并通过验收前，禁止将 8000 端口暴露到局域网或公网。
>
...
```bash
curl -X POST http://127.0.0.1:8000/documents/upload -F "file=@手册.pdf"
```
...
```powershell
Invoke-RestMethod -Uri http://127.0.0.1:8000/qa/ask -Method Post -ContentType "application/json" -Body '{"question":"报销流程是什么？","user_id":"u1","session_id":"s1"}'
# 返回: {"answer":"...[来源:手册.pdf 第3页]","sources":["手册.pdf 第3页"],"refused":false,"need_human":false}
```

**会话历史**
```bash
curl http://127.0.0.1:8000/qa/history/s1
```

**管理看板**（统计 / 拒答列表 / 转人工列表）
```bash
curl http://127.0.0.1:8000/admin/stats
curl http://127.0.0.1:8000/admin/refused
curl http://127.0.0.1:8000/admin/human
```
