...
### 实测结果（2026-08-24，真实 pgvector + 百炼 text-embedding-v3）

**检索层**（115 题，OOS 由端到端考核）：

| 指标 | 基线（词典 63 组 / 阈值 0.80） | 迭代后（词典 80 组 / 阈值 0.55） |
|------|------|------|
| 检索 hit@5 | 93.0% (107/115) | **95.7% (110/115)** |
| MRR | 0.820 | **0.827** |
| L2 口语改写 | 85.7% | **92.9%** |

**端到端层**（121 题，走真实 /qa/ask：拒答行为 + 关键词覆盖 + 来源一致三关）：
...
迭代手段：① 同义词典按"口语问句子串 → 规范政策词"方向修正 17 组映射；② 证据距离阈值由黄金集分布实测校准（正常命中 top1 p95=0.44，收紧至 0.55 零误杀）；③ 出题错误以语料原文为准修正（澳洲时效、B2B 折扣梯度）。剩余未命中为跨文档组合查询与邮箱打码环境副作用，属已知边界。
